...
namespace StreamDeckSharp.Internals
{
  internal class BasicHidClient : IStreamDeckBoard
  {
    ...
    private readonly BehaviorSubject<KeyEventArgs> keyStateChanged = new BehaviorSubject<KeyEventArgs>(null);
    ...
    public BasicHidClient(IStreamDeckHid deckHid, IHardwareInternalInfos hardwareInformation)
    {
      ...
      ConnectionStateChanged = Observable
        .FromEventPattern<ConnectionEventArgs>(deckHid, nameof(ConnectionStateChanged))
    ...
    public IObservable<ConnectionEventArgs> ConnectionStateChanged { get; private set; }
    ...
    private void ProcessKeys(byte[] newStates)
    {
      for (var i = 0; i < keyStates.Length; i++)
      {
        ...
        if (keyStates[i] != newStates[newStatePos])
        {
          ...
          keyStateChanged.OnNext(new KeyEventArgs(externalKeyId, newStates[newStatePos] != 0));
